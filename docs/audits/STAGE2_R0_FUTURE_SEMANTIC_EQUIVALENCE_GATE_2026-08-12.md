...
| Pair | Stage 2 relation audit | Governance distinction | Temporal-boundary distinction | Gate |
|---|---|---|---|---|
| Wu Zetian / Wu Zhou | `rules` / `rules` | none | none | PASS |
| Sejong / Joseon | `rules` / `rules` | none | none | PASS |
| Mehmed II / Ottoman Empire | `rules` / `rules` | none | none | PASS |
| Charles V / Holy Roman Empire | `rules` / `rules` | none | none | PASS |
| Simón Bolívar / Gran Colombia | `rules` / `rules` | none | none | PASS |
| Bismarck / German Empire | `governs` / `governs` | none | none | PASS |
