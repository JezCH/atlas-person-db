...
function evaluateFkSurface(label, actualRows, expectedRows, blockers) {
  ...
  for (const [key, rule] of expected) {
    ...
    if (!row) {
      blockers.push(`${label}_FK_MISSING:${key}`);
      continue;
    }
    if (row.columns.length !== 1) blockers.push(`${label}_FK_NOT_SINGLE_COLUMN:${key}`);
    if (row.delete_action !== rule.delete_action) {
      blockers.push(`${label}_FK_DELETE_ACTION_DRIFT:${key}:${row.delete_action}->${rule.delete_action}`);
    }
  }
  for (const key of actual.keys()) {
    if (!expected.has(key)) blockers.push(`${label}_FK_UNREVIEWED:${key}`);
  }
}
...
async function inspectPersonMergeReferenceReadiness(client) {
  ...
  for (const column of difference(nonFkPersonUuidColumns, EXPECTED_NON_FK_PERSON_UUID_COLUMNS)) {
    blockers.push(`PERSON_UUID_REFERENCE_UNREVIEWED:${column}`);
  }
  for (const column of difference(EXPECTED_NON_FK_PERSON_UUID_COLUMNS, nonFkPersonUuidColumns)) {
    blockers.push(`PERSON_UUID_SNAPSHOT_MISSING:${column}`);
  }
  for (const column of difference(nonFkRelationshipUuidColumns, EXPECTED_NON_FK_RELATIONSHIP_UUID_COLUMNS)) {
    blockers.push(`RELATIONSHIP_UUID_REFERENCE_UNREVIEWED:${column}`);
  }
  for (const column of difference(EXPECTED_NON_FK_RELATIONSHIP_UUID_COLUMNS, nonFkRelationshipUuidColumns)) {
    blockers.push(`RELATIONSHIP_UUID_SNAPSHOT_MISSING:${column}`);
  }
  for (const trigger of userTriggers) blockers.push(`MERGE_SURFACE_TRIGGER_UNREVIEWED:${trigger}`);

  return Object.freeze({
    policy_version: PERSON_REFERENCE_POLICY_VERSION,
    ready: blockers.length === 0,
    blockers: Object.freeze(blockers.sort()),
    person_fks: Object.freeze(personFks),
    relationship_fks: Object.freeze(relationshipFks),
    non_fk_person_uuid_columns: Object.freeze(nonFkPersonUuidColumns),
    non_fk_relationship_uuid_columns: Object.freeze(nonFkRelationshipUuidColumns),
...
module.exports = Object.freeze({
  PERSON_REFERENCE_POLICY_VERSION,
  EXPECTED_PERSON_FKS,
  EXPECTED_RELATIONSHIP_FKS,
  EXPECTED_NON_FK_PERSON_UUID_COLUMNS,
  EXPECTED_NON_FK_RELATIONSHIP_UUID_COLUMNS,
  inspectPersonMergeReferenceReadiness,
