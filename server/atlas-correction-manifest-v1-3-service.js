...
function requireOperation(raw, index) {
  ...
  if (normalizeText(raw.type) !== OPERATION_TYPE) throw new Error("CORRECTION_V13_OPERATION_UNSUPPORTED");
  ...
  return Object.freeze({ type: OPERATION_TYPE, person_id: personId, expected_before: before, expected_after: after });
}
...
function requireManifest(raw) {
  ...
  for (const operation of operations) {
    const key = `${operation.person_id}|${operation.expected_before.locale}`;
    if (seen.has(key)) throw new Error("CORRECTION_V13_PERSON_LOCALE_REUSED");
    seen.add(key);
  }
...
function createCorrectionManifestV13Service({ client } = {}) {
  ...
  async function execute(rawManifest, { dryRun = false } = {}) {
    ...
    try {
      ...
      const outcomes = [];
      for (const operation of manifest.operations) outcomes.push(await replacePreferredName(client, operation));
