...
const MODES = new Set(["preflight","entities_dry_run","entities_apply","roles_dry_run","roles_apply","source_dry_run","source_apply","polity_dry_run","polity_apply","governance_dry_run","governance_apply","correction_dry_run","correction_apply","p9_dry_run","p9_apply","final_verify"]);
...
function createStage2Train2ReleaseHandler({env=process.env,verifyOidc=verifyGitHubActionsOidc,createClient=createPostgresClient}={}){
 return async function handler(req,res){
  ...
  let client;try{
    ...
    if(p.mode==="preflight"){await requireP5Schema(client);result={schema_ready:true,p9:await inspectP9Cutover(client),person_merge:personMergeExecutionState()};}
      ...
      const p9=await inspectP9Cutover(client),merge=personMergeExecutionState();if(p9.old_index_present||!p9.new_index_present||p9.duplicate_groups!==0)throw new Error("TRAIN2_P9_FINAL_VERIFY_FAILED");if(merge.allowed!==false||merge.person_merge_lifecycle_version!=="pre-p10-blocked")throw new Error("TRAIN2_P10_INTERLOCK_FAILED");result={p9,person_merge:merge};
    }
