...
function synthesizeUnifiedCorrectionV2Manifest(plan, snapshot) {
  ...
  const normalizedAssertions = rawAssertions.map((raw, index) => {
    const type = String(raw?.type || "").trim();
    if (!STAGE2_ASSERTION_TYPES.has(type)) throw new Error("CORRECTION_V2_STAGE2_ASSERTION_OPERATION_UNSUPPORTED");
    return normalizeStage2AssertionOperation(raw, baseCore.operations.length + index + 1);
  });
  const sourceAssertions = normalizedAssertions.filter((operation) => operation.type === "assert_source");
  const otherAssertions = normalizedAssertions.filter((operation) => operation.type !== "assert_source");
  const operations = [...sourceAssertions, ...baseCore.operations, ...otherAssertions];
