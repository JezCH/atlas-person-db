...
const EXPECTED_AUDIENCE = "atlas-person-db-ko-repair";
const EXPECTED_REPOSITORY = "JezCH/atlas-person-db";
const EXPECTED_REPOSITORY_ID = "1319427399";
const EXPECTED_REF = "refs/heads/main";
const EXPECTED_WORKFLOW_REF = "JezCH/atlas-person-db/.github/workflows/atlas-authoritative-ko-repair.yml@refs/heads/main";
...
function decodeJsonPart(value) {
  ...
  catch { throw new Error("GITHUB_OIDC_MALFORMED_TOKEN"); }
}
...
async function fetchJwks({ fetchImpl = globalThis.fetch, now = Date.now } = {}) {
  ...
  if (!Array.isArray(body?.keys) || !body.keys.length) throw new Error("GITHUB_OIDC_JWKS_INVALID");
  jwksCache = body.keys;
  jwksCachedAt = now();
  return jwksCache;
}
...
async function verifyGitHubActionsOidc(token, { expectedSha, fetchImpl = globalThis.fetch, now = Date.now } = {}) {
  ...
  catch { throw new Error("GITHUB_OIDC_SIGNING_KEY_INVALID"); }
