...
async function loadRevalidationRequirements(client) {
  ...
  if (!table.rows[0]?.requirements) return { schema_ready: false, rows: [] };
...
async function loadDetectorInput(client) {
  ...
  `);
  const requirements = await loadRevalidationRequirements(client);
  return {
    names: names.rows || [],
    activities: activities.rows || [],
    requirements: requirements.rows,
    requirements_schema_ready: requirements.schema_ready
  };
}
...
async function listCandidates({ client, includeStale = false } = {}) {
  ...
  if (personIds.length) {
    const names = await client.query(`
      select person_id,name,locale,is_preferred
       ...
       order by person_id,is_preferred desc,locale,name`, [personIds]);
    ...
    const activities = await client.query(`
      select pp.person_id,pp.id,pp.polity_id,pp.relation_type_id,pp.role_id,pp.period_basis_id,
        ...
        join atlas_v2.polity_names pen on pen.polity_id=pp.polity_id and pen.locale='en' and pen.is_preferred=true
        ...
        left join lateral (
          select rn.name from atlas_v2.role_names rn
           where rn.role_id=r.id and rn.locale='ko' and rn.is_preferred=true
           order by rn.id limit 1
        ) rko on true
        join atlas_v2.period_bases pb on pb.id=pp.period_basis_id
       where pp.person_id=any($1::uuid[])
       order by pp.person_id,pp.activity_start,pp.activity_end,polity_name,pp.id`, [personIds]);
    for (const row of activities.rows || []) {
      const key = String(row.person_id);
      const list = activitiesByPerson.get(key) || [];
      list.push({
        id: String(row.id), person_id: key, polity_id: String(row.polity_id), polity_name: String(row.polity_name),
        ...
        role_name: row.role_name == null ? null : String(row.role_name), role_name_en: row.role_name_en == null ? null : String(row.role_name_en),
        period_basis_id: String(row.period_basis_id), period_basis: String(row.period_basis),
  ...
  const decorate = (personId) => {
    ...
    return { id: personId, display_name: preferredName(names) || personId, names, activities: activitiesByPerson.get(personId) || [] };
  };
  ...
  const candidates = rows.map((row) => {
    ...
    const relationshipGroups = buildRelationshipReconciliationGroups({ rows: [...low.activities,...high.activities], lowPersonId: lowId, highPersonId: highId }).map((group) => ({
      ...group,
    ...
    return {
      ...
      first_detected_at: row.first_detected_at,last_detected_at: row.last_detected_at,reviewed_at: row.reviewed_at,review_count: Number(row.review_count),
      relationship_reconciliation: { semantic_version: REVALIDATION_SEMANTIC_VERSION, required: relationshipGroups.length > 0, groups: relationshipGroups },
      low,high
    };
  });
...
async function reviewCandidate({ client,candidateId,decision,rationale=null,requestId,reviewerKind="admin_session" }) {
  ...
  if (!/^[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}$/i.test(normalizedCandidateId)) throw new Error("valid candidate_id is required");
  ...
  try {
    ...
    const locked = await client.query(`
      select id,person_low_id,person_high_id,candidate_state,evidence,evidence_fingerprint,detector_version
        from atlas_v2.person_duplicate_candidates where id=$1 for update`, [normalizedCandidateId]);
    ...
    if (String(candidate.detector_version) !== DETECTOR_VERSION) throw new Error("candidate detector version is stale; rebuild before review");

    await client.query(`
      insert into atlas_v2.person_duplicate_reviews (
        id,candidate_id,person_low_id,person_high_id,decision,rationale,evidence_snapshot,evidence_fingerprint,reviewer_kind,request_id,reviewed_at
      ) values ($1,$2,$3,$4,$5,$6,$7::jsonb,$8,$9,$10,now())`, [
        crypto.randomUUID(),normalizedCandidateId,candidate.person_low_id,candidate.person_high_id,normalizedDecision,rationaleText,
        JSON.stringify(candidate.evidence),candidate.evidence_fingerprint,reviewerKind === "server_bearer" ? "server_bearer" : "admin_session",normalizedRequestId
      ]);
    await client.query(`
      update atlas_v2.person_duplicate_candidates
         set current_decision=$2,decision_evidence_fingerprint=evidence_fingerprint,reviewed_at=now(),review_count=review_count+1,updated_at=now()
       where id=$1`, [normalizedCandidateId,normalizedDecision]);
    await client.query("COMMIT");
