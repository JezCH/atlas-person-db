(() => {
  ...
  function breakdownCard(label, item, { eyebrow = "INCOMPLETE REASONS" } = {}) {
    ...
    const rows = (item?.rows || []).map((row) => `<div class="dashboard-reason-row" data-reason-code="${escapeHtml(row.code || "")}">
  ...
  function renderSnapshot(root, snapshot) {
    ...
    root.innerHTML = `<section class="dashboard-control-center">
      ...
      <section class="dashboard-kpi-grid" aria-label="핵심 통계">
        ...
        ${kpiCard({code:"namuwiki",label:"NAMUWIKI REVIEW",primary:pct(w.namuwiki.percentage),detail:`연결 ${value(w.namuwiki.linked)} · 없음 확정 ${value(w.namuwiki.confirmed_absent)} · 문서 확인 보류 ${value(w.namuwiki.pending)} · 검증 필요 ${value(w.namuwiki.remaining)}`,drilldown:kd.namuwiki})}
      ...
      <section class="dashboard-lower-grid" aria-label="NamuWiki 없음 확정 세부">
        ${breakdownCard("NamuWiki 없음 확정 세부", namuwikiAbsence, { eyebrow:"NAMUWIKI ABSENCE BREAKDOWN" })}
      </section>
