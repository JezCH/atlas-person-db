(() => {
  ...
  function renderInto(mount) {
    ...
    const capitalPolityCount = Array.isArray(capitalIndex?.records) ? capitalIndex.records.length : 0;
    const capitalPeriodCount = Array.isArray(capitalIndex?.records) ? capitalIndex.records.reduce((sum, record) => sum + (Array.isArray(record?.capital_periods) ? record.capital_periods.length : 0), 0) : 0;
    ...
    <section class="spacetime-status-row">
      <span><b>${entries.length}</b> Activity</span><span><b>${placedCount}</b> 배치 구간</span><span><b>${placement.unresolvedPosition.length}</b> 위치 미확정</span><span><b>${placement.unresolvedChronology.length}</b> 연대 미확정</span><span><b>${capitalPolityCount}</b> 수도 연결 Polity</span><span><b>${capitalPeriodCount}</b> 검증 수도 구간</span>
    </section>
    ${capitalPeriodCount === 0 ? `<section class="spacetime-integrity-note card"><strong>위치 데이터를 추측하지 않았습니다.</strong><p>현재 Person DB에는 시공간 인물도가 사용할 수 있는 검증된 수도 시계열이 아직 없습니다. 따라서 이름·현대국가·출생지로 임의 배치하지 않고 수도 미연결 Activity를 아래 ‘위치 미확정’에 보존합니다. source-reviewed 수도 기록이 추가되면 같은 화면에 자동 배치됩니다.</p></section>` : ""}
    ...
    <section class="spacetime-unresolved-grid">
      ...
      <article class="card"><div class="spacetime-unresolved-head"><div><p class="eyebrow">CHRONOLOGY REVIEW</p><h3>연대 미확정</h3></div><strong>${placement.unresolvedChronology.length}</strong></div><p>Activity 시작·종료 연도를 둘 다 확정할 수 없는 경우 세로축에 억지 기간을 만들지 않습니다.</p>${unresolvedRows(placement.unresolvedChronology)}</article>
    </section>`;
