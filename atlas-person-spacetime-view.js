(() => {
  ...
  function renderInto(mount) {
    ...
    <section class="spacetime-unresolved-grid">
      <article class="card"><div class="spacetime-unresolved-head"><div><p class="eyebrow">PLACEMENT REVIEW</p><h3>위치 미확정</h3></div><strong>${placement.unresolvedPosition.length}</strong></div><p>검토된 정치체 권역·정치체 장소 기능으로 가로 위치를 확정할 수 없어 임의 배치를 하지 않은 Activity입니다.</p>${unresolvedRows(placement.unresolvedPosition)}</article>
