...
if (raw.schema !== 'atlas-stage2-domain-contract/v1') throw new Error(`unexpected Stage 2 domain contract schema: ${raw.schema}`);
if (raw.production_migration_authorized !== false) throw new Error('Stage 2 domain contract must remain explicitly non-production');
...
function uniqueCodes(items, label) {
  ...
  if (values.some((value) => typeof value !== 'string' || value.length === 0)) throw new Error(`${label} contains an invalid code`);
  if (new Set(values).size !== values.length) throw new Error(`${label} contains duplicate codes`);
...
export const polityNameSemanticKinds = uniqueCodes(raw.polity_name_semantic_kinds, 'polity_name_semantic_kinds');
export const peopleGroupTypes = uniqueCodes(raw.people_group_types, 'people_group_types');
export const personPeopleAffiliationTypes = uniqueCodes(raw.person_people_affiliation_types, 'person_people_affiliation_types');
export const historicalEventTypes = uniqueCodes(raw.historical_event_types, 'historical_event_types');
export const personEventParticipationTypes = uniqueCodes(raw.person_event_participation_types, 'person_event_participation_types');
...
export const entityBoundaryProvenanceSourceJoinTables = uniqueCodes(raw.entity_boundary_provenance_source_join_tables, 'entity_boundary_provenance_source_join_tables');
