...
const mergeService=fs.readFileSync('server/atlas-person-merge-service.js','utf8');
...
assert.equal(interlock.personMergeExecutionState().allowed,true);
assert.equal(interlock.personMergeExecutionState().person_merge_lifecycle_version,'p10-v2-revalidated');
assert.match(mergeService,/assertPersonDuplicateRevalidationReadiness/);
