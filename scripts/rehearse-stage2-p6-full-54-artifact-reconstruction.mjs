...
function validatePackage(plans, baseline) {
  ...
  const mutationPlanById = new Map();
  plans.forEach((plan, planIndex) => {
    for (const operation of plan.operations || []) mutationPlanById.set(String(operation.activity_id).toLowerCase(), { planIndex, batchId: plan.batch_id, caseId: operation.case_id });
  });

  const createdIds = [];
  const crossPlanRetireDependencies = [];
  plans.forEach((plan, planIndex) => {
    ...
    for (const operation of plan.operations || []) {
      if (operation.type === 'retire_activity') {
        for (const replacementId of operation.replacement_activity_ids || []) {
          const replacement = String(replacementId).toLowerCase();
          if (mutationSet.has(replacement)) {
            const producer = mutationPlanById.get(replacement);
            assert.ok(producer, `retire replacement mutation producer missing: ${replacementId}`);
            assert.notEqual(producer.planIndex, planIndex, `retire replacement cannot be another mutation in the same atomic plan: ${replacementId}`);
            assert.ok(producer.planIndex < planIndex,
              `retire replacement correction must precede retire plan: survivor=${replacementId} producer=${producer.batchId} retire=${plan.batch_id}`);
            crossPlanRetireDependencies.push({ survivor_activity_id: replacement, survivor_case_id: producer.caseId, survivor_batch_id: producer.batchId, retire_case_id: operation.case_id, retire_batch_id: plan.batch_id });
          }
        }
      }
  ...
  return Object.freeze({ operations, mutationIds, createdIds, requiredIds, baselineById, crossPlanRetireDependencies });
}
...
try {
  ...
  console.log(JSON.stringify({
    ...
    plans_rehearsed: plans.length, activity_targets: packageInfo.mutationIds.length, golden_activity_targets: 9,
    materialized_activity_targets: literalPackage.result.compiled_activity_targets, split_created_activity_rows: packageInfo.createdIds.length,
    split_operations: splitOperations, retire_operations: retireOperations, cross_plan_retire_dependencies_verified: packageInfo.crossPlanRetireDependencies.length,
