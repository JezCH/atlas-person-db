...
function parseLegacySourceKey(key) {
  ...
  if (!match) throw new Error(`P7_SINGLE_REWRITE_LEGACY_SOURCE_KEY_INVALID:${key}`);
...
function assertPlanMatchesBaseline(operation, row) {
  const fields = ['person_id','polity_id','role_id','period_basis_id','activity_start','activity_end','confidence','chronology_status','legacy_source_key'];
  for (const field of fields) assert.equal(operation.baseline_before[field], row[field], `plan baseline drift ${field}`);
  assert.equal(Number(operation.baseline_before.source_count), Number(row.source_count), 'plan baseline source_count drift');
}

export async function rehearseP7SingleRewrite({
  planPath,
  sourceManifestPath,
  marker,
  verifyAfter
  ...
  assert.equal(plan.operations.length, 1, 'generic single rewrite rehearsal requires exactly one operation');
  ...
  try {
    ...
    const sourceResult = await applyReviewedSourceAuthoring(client, { manifestPath: path.resolve(root, sourceManifestPath) });
    assert.equal(sourceResult.replay, false);
    const sourceReplay = await applyReviewedSourceAuthoring(client, { manifestPath: path.resolve(root, sourceManifestPath) });
    ...
    const actual = (await client.query(ACTIVITY_SELECT, ids)).rows[0];
    const sourceLinks = (await client.query(`select person_politics_id::text,source_id::text,source_locator_key from atlas_v2.person_politics_sources where person_politics_id=$1::uuid order by source_id::text,source_locator_key`, ids)).rows;
    assert.ok(actual);
    assert.equal(actual.id, operation.activity_id);
    assert.equal(sourceLinks.length, 1 + operation.after.add_source_links.length);
    if (verifyAfter) await verifyAfter({ actual, sourceLinks, baselineRow: row, operation, plan, client });
    const result = { marker, activity_id: operation.activity_id, reviewed_sources_added: operation.after.add_source_links.length, dry_run_apply_replay_verified: true, production_mutation_authorized: false, production_or_vercel_contacted: false };
