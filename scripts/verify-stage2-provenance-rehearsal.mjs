...
try {
  ...
  if (Object.values(prereq).some((value) => !value)) {
    throw new Error(`Stage 2 semantic rehearsal must run before provenance rehearsal: ${JSON.stringify(prereq)}`);
  }
  ...
  same(tables.rows.map((r) => r.table_name), expectedTables, 'provenance tables');
  ...
  const indexes = await client.query(`
    select indexname
     ...
     order by indexname`, [expectedSourceIndexes]);
  same(indexes.rows.map((r) => r.indexname), expectedSourceIndexes, 'source reverse-lookup indexes');
  ...
  const fkRows = await client.query(`
    select con.conname, confdeltype
     ...
     order by con.conname`, [[
      'polity_governance_period_sources_period_id_fkey',
      'polity_governance_period_sources_source_id_fkey',
      'polity_relation_sources_relation_id_fkey',
      'polity_relation_sources_source_id_fkey',
      'polity_designation_sources_designation_id_fkey',
      'polity_designation_sources_source_id_fkey',
      'polity_identity_relation_sources_relation_id_fkey',
      'polity_identity_relation_sources_source_id_fkey'
    ]]);
  ...
  for (const row of fkRows.rows) {
    ...
    const expectedDelete = sourceFk ? 'r' : 'c'; // RESTRICT / CASCADE
    if (row.confdeltype !== expectedDelete) {
      throw new Error(`${row.conname} delete behavior drift: expected ${expectedDelete}, got ${row.confdeltype}`);
    }
  }

  const locatorChecks = await client.query(`
    select count(*)::int as n
     ...
     where n.nspname='atlas_v2'
       and con.contype='c'
       and con.conname = any($1::text[])`, [[
      'polity_governance_period_sources_locator_check',
      'polity_relation_sources_locator_check',
      'polity_designation_sources_locator_check',
      'polity_identity_relation_sources_locator_check'
    ]]);
  if (locatorChecks.rows[0]?.n !== 4) throw new Error(`locator check contract incomplete: ${locatorChecks.rows[0]?.n}`);
  ...
  try {
    ...
    await client.query(`insert into atlas_v2.polity_governance_periods(id,polity_id,governance_context_id,valid_from_year,valid_from_granularity,valid_from_certainty,valid_from_calendar,valid_to_year,valid_to_granularity,valid_to_certainty,valid_to_calendar,confidence) values ($1,$2,$3,1,'year','exact','unspecified_historical',2,'year','exact','unspecified_historical','well_established')`, [governancePeriod, polityA, governance]);
    await client.query(`insert into atlas_v2.polity_relations(id,subject_polity_id,object_polity_id,relation_type_id,valid_from_year,valid_from_granularity,valid_from_certainty,valid_from_calendar,valid_to_year,valid_to_granularity,valid_to_certainty,valid_to_calendar,confidence) values ($1,$2,$3,$4,1,'year','exact','unspecified_historical',2,'year','exact','unspecified_historical','well_established')`, [polityRelation, polityA, polityB, relationType]);
    await client.query(`insert into atlas_v2.polity_designations(id,polity_id,designation_type,valid_from_year,valid_from_granularity,valid_from_certainty,valid_from_calendar,valid_to_year,valid_to_granularity,valid_to_certainty,valid_to_calendar,confidence) values ($1,$2,'state_form',1,'year','exact','unspecified_historical',2,'year','exact','unspecified_historical','well_established')`, [designation, polityA]);
    await client.query(`insert into atlas_v2.polity_identity_relations(id,predecessor_polity_id,successor_polity_id,relation_type_id,transition_year,transition_granularity,transition_certainty,transition_calendar,confidence) values ($1,$2,$3,$4,2,'year','exact','unspecified_historical','well_established')`, [identityRelation, polityA, polityB, identityRelationType]);
    ...
    await client.query(`insert into atlas_v2.polity_relation_sources values ($1,$2,'section:relation')`, [polityRelation, source]);
    ...
    if (l.governance_links !== 1 || l.relation_links !== 1 || l.designation_links !== 1 || l.identity_links !== 1) {
      throw new Error(`provenance insert probe failed: ${JSON.stringify(l)}`);
    }

    await client.query('SAVEPOINT blank_locator_probe');
    let blankRejected = false;
    try {
      await client.query(`update atlas_v2.polity_relation_sources set source_locator_key='   ' where polity_relation_id=$1 and source_id=$2`, [polityRelation, source]);
    } catch (error) {
    ...
    const relationLinksAfterDelete = await client.query(`select count(*)::int as n from atlas_v2.polity_relation_sources where polity_relation_id=$1`, [polityRelation]);
    if (relationLinksAfterDelete.rows[0]?.n !== 0) throw new Error('assertion delete did not cascade provenance join');
  } finally {
  ...
  if (legacy.rows[0]?.legacy_table || legacy.rows[0]?.legacy_compat) throw new Error('legacy objects appeared during provenance rehearsal');

  console.log(JSON.stringify({
    marker: 'ATLAS_STAGE2_PROVENANCE_REHEARSAL_V1',
    status: 'PASS',
    ...
    assertion_delete_cascades_link: true,
    cited_source_delete_restricted: true,
    source_reverse_lookup_indexes: expectedSourceIndexes.length,
