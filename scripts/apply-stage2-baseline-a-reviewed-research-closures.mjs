...
const expectedReviewedCount = Number(closures.result?.reviewed_generic_research_labels_closed || 0) + Number(residual.result?.residual_cases_total || 0);
if (byActivity.size !== expectedReviewedCount || expectedReviewedCount !== 36) throw new Error(`combined reviewed case count drift: expected 36, got ${byActivity.size}`);
