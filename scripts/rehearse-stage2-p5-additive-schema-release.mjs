...
if(release.release.prerequisites?.effective_prebinding_complete!==true||release.release.prerequisites?.effective_activity_count!==54||release.release.safety?.production_apply_authorized!==false) throw new Error('P5 release prerequisite drift');
for(const component of release.components){
  const normalized=component.body.replace(/^\s*--.*$/gm,'');
  if(/\b(?:delete|update)\s+atlas_v2\.person_politics_v2\b/i.test(normalized)||/\btruncate\b/i.test(normalized)||/\bdrop\s+(?:table|schema)\b/i.test(normalized)) throw new Error(`non-additive component ${component.id}`);
...
try{
  ...
  if(first.applied.length!==5||first.skipped.length!==0) throw new Error(`first release apply drift ${JSON.stringify(first)}`);
  const second=await applyStage2SchemaRelease(client);
  if(second.applied.length!==0||second.skipped.length!==5) throw new Error(`release replay drift ${JSON.stringify(second)}`);
  ...
  if(ledger.rowCount!==5) throw new Error('Stage2 release ledger incomplete');
  ...
  console.log(JSON.stringify({marker:'ATLAS_STAGE2_P5_ADDITIVE_SCHEMA_RELEASE_REHEARSAL_OK',release_id:release.release.release_id,components:5,first_apply:5,replay_skipped:5,person_relation_types:6,polity_relation_types:3,production_mutation_authorized:false}));
