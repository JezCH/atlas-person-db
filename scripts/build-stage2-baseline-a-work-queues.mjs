...
const summary={schema:'atlas-stage2-baseline-a-work-queues-summary/v2',baseline:ledger.baseline,baseline_relationships:ledger.rows.length,unique_activity_ids:uniqueIds.size,reviewed_research_closures_applied:Number(ledger.summary?.reviewed_research_closures_applied||0),reviewed_model_refinements_applied:Number(ledger.summary?.reviewed_model_refinements_applied||0),person_identity_decisions_applied:Number(ledger.summary?.person_identity_decisions_applied||0),person_identity_decisions_unresolved:Number(ledger.summary?.person_identity_decisions_unresolved||0),p10_physical_person_merges_required:Number(ledger.summary?.p10_physical_person_merges_required||0),physical_person_merges_performed:Number(ledger.summary?.physical_person_merges_performed||0),p4_polity_identity_dependency_total:p4PolityTotal,p4_polity_identity_decisions_applied:p4PolityApplied,p4_polity_identity_decisions_unresolved:p4PolityUnresolved,p4_polity_identity_decided_execution_pending:p4PolityExecutionPending,dependency_counts:dependencyCounts,execution_class_counts:executionCounts,current_schema_cleanup_pending:0,production_mutation_authorized:false};
...
console.log(`ATLAS_BASELINE_A_WORK_QUEUES_OK rows=${ledger.rows.length} cleanup_pending=0 research=${dependencyCounts.historical_research} person_identity=${dependencyCounts.person_identity_review} polity_identity_unresolved=${p4PolityUnresolved} polity_identity_decided_execution_pending=${p4PolityExecutionPending}`);console.log(JSON.stringify(summary,null,2));
