...
function baselineSourceEvidence(row, sourceByTitle) {
  ...
  return Object.freeze({
    activity: Object.freeze({
      id: row.activity_id, person_id: row.person_id, polity_id: row.polity_id, role_id: row.role_id,
      period_basis_id: row.period_basis_id, activity_start: row.activity_start, activity_end: row.activity_end,
      confidence: row.confidence, chronology_status: row.chronology_status, legacy_source_key: row.legacy_source_key,
      notes: row.notes, source_locator: Object.freeze({ file: parsed.file, index: parsed.index, source_sha256: source.sha256 }),
      // Baseline A captured only the 16-hex content-hash prefix in legacy_source_key. Never fabricate the missing suffix.
...
function validatePackage(reviewedPlans, executionUnits, baseline) {
  assert.equal(reviewedPlans.length, 18, 'P6 full rehearsal must contain golden Batch1 plus 17 materialized reviewed plans');
  assert.ok(executionUnits.length >= reviewedPlans.length, 'execution wave scheduler may preserve or expand reviewed plans, never drop them');
  const operations = reviewedPlans.flatMap((plan) => plan.operations || []);
  ...
  assert.equal(mutationSet.size, 54, 'P6 full rehearsal Activity targets must be UUID-unique');
  ...
  for (const plan of reviewedPlans) {
    ...
    for (const operation of plan.operations || []) {
      if (operation.type === 'split_activity') {
        for (const fragment of operation.fragments || []) if (fragment.survivor !== true) createdIds.push(String(fragment.activity_id).toLowerCase());
      }
    }
  }
  ...
  const unitMutationIds = executionUnits.flatMap((unit) => (unit.operations || []).map((operation) => String(operation.activity_id).toLowerCase()));
  assert.equal(unitMutationIds.length, 54, 'execution waves must preserve every reviewed Activity mutation exactly once');
  assert.deepEqual([...unitMutationIds].sort(), [...mutationIds].sort(), 'execution waves changed the reviewed Activity target set');
  const requestIds = executionUnits.map((unit) => unit.batch_id);
  assert.equal(new Set(requestIds).size, requestIds.length, 'execution wave request ids must be unique');

  const requiredIds = [...new Set(executionUnits.flatMap((unit) => requiredSnapshotActivityIds(unit)))].sort();
  ...
  return Object.freeze({ operations, mutationIds, createdIds, requiredIds, baselineById, requestIds });
}
...
const reviewedPlans = Object.freeze([goldenPlan, ...literalPackage.plans]);
const schedule = scheduleCorrectionPlans(reviewedPlans);
assert.equal(schedule.reviewed_plan_count, 18);
const executionUnits = schedule.execution_units;
const packageInfo = validatePackage(reviewedPlans, executionUnits, baseline);
...
try {
  ...
  assert.deepEqual(seeded.rows[0], { persons: 302, polities: 212, roles: 149, period_bases: 8, sources: 20, activities: packageInfo.requiredIds.length, activity_sources: packageInfo.requiredIds.length });
  ...
  let dryRuns = 0, applies = 0, replays = 0, manifestOperations = 0, relationAssertions = 0, retireOperations = 0, splitOperations = 0;

  for (const unit of executionUnits) {
    const snapshotIds = requiredSnapshotActivityIds(unit);
    const snapshot = await createCorrectionV2TargetSnapshot(client, snapshotIds);
    const manifest = synthesizeCorrectionV2Manifest(unit, snapshot);
    ...
    for (const operation of manifest.operations) {
      ...
      if (operation.type === 'assert_polity_relation') {
        ...
        assert.ok(relationId, `relation assertion without literal UUID in ${unit.batch_id}`);
    ...
    assert.equal(afterDrySnapshot.snapshot_digest, beforeDryDigest, `dry-run changed target snapshot for ${unit.batch_id}`);
  ...
  assert.equal(finalCounts.rows[0].correction_runs, executionUnits.length);
  ...
  console.log(JSON.stringify({
    ...
    baseline: { deployment_sha: BASELINE_SHA, baseline_digest: BASELINE_DIGEST, captured_activity_rows: 338, exact_identity_catalogs_used: true,
      exact_activity_baseline_tuple_fields_used: true, required_target_and_dependency_rows: packageInfo.requiredIds.length, required_rows_single_normalized_source_link: true,
      normalized_source_uuid_and_locator_reconstructed_from_legacy_key_and_source_catalog: true, full_content_hash_value_available_in_baseline_artifact: false,
      content_hash_reconstruction_uses_captured_legacy_hash_prefix_only: true },
    reviewed_plans: reviewedPlans.length, execution_units: executionUnits.length, dependency_wave_expansion: executionUnits.length - reviewedPlans.length,
    cross_unit_retire_dependencies_verified: schedule.cross_unit_retire_dependencies.length,
    activity_targets: packageInfo.mutationIds.length, golden_activity_targets: 9, materialized_activity_targets: literalPackage.result.compiled_activity_targets,
    split_created_activity_rows: packageInfo.createdIds.length, split_operations: splitOperations, retire_operations: retireOperations,
    relation_assertions: relationAssertions, synthesized_manifest_operations: manifestOperations, dry_runs_rolled_back: dryRuns,
    manifests_applied_atomically: applies, exact_replays_verified: replays, correction_ledger_rows: finalCounts.rows[0].correction_runs,
    production_mutation_authorized: false, production_or_vercel_contacted: false
  }, null, 2));
} finally {
  await client.end();
}
