...
if (/certainty/i.test(ddl.replace(/^\s*--.*$/gm, ''))) throw new Error('temporal certainty must not enter Activity semantic identity');
if (/\bconfidence\b/i.test(ddl.replace(/^\s*--.*$/gm, ''))) throw new Error('confidence must not enter Activity semantic identity');
if (/\bnotes\b/i.test(ddl.replace(/^\s*--.*$/gm, ''))) throw new Error('notes must not enter Activity semantic identity');
...
try {
  const stage2Columns = await client.query(`
    select column_name
     ...
     where table_schema='atlas_v2'
       ...
       and column_name in (
         'relation_type_id','activity_start_granularity','activity_start_calendar',
  ...
  const indexResult = await client.query(`
    select indexdef
     ...
     where schemaname='atlas_v2'
       and indexname='person_politics_v2_stage2_semantic_identity_uq'`);
  ...
  for (const token of requiredTokens) {
    if (!indexDef.includes(token)) throw new Error(`semantic identity index missing ${token}`);
  }
  if (!/NULLS NOT DISTINCT/i.test(indexDef)) throw new Error('semantic identity index must use NULLS NOT DISTINCT');
  for (const forbidden of ['certainty','confidence','notes','source_locator','content_hash']) {
    if (indexDef.includes(forbidden)) throw new Error(`evidence/content field leaked into semantic identity: ${forbidden}`);
  }
  ...
  try {
    ...
    try {
      await insert({
        ...
        relationTypeId: rulesId,
        startCertainty: 'exact',
        endCertainty: 'exact',
        confidence: 'well_established',
        notes: 'different evidence/content'
      });
    } catch (error) {
    ...
    await insert({
      id: '00000000-0000-4000-8000-000000000714', relationTypeId: rulesId,
      startMonth: 5, startDay: 22, startGranularity: 'day',
      endMonth: 12, endDay: 10, endGranularity: 'day'
    });

    await insert({
      id: '00000000-0000-4000-8000-000000000715', relationTypeId: rulesId,
      startMonth: 5, startDay: 22, startGranularity: 'day', startCalendar: 'julian',
    ...
    const counts = await client.query(`
      ...
      where person_id=$3`, [rulesId, servesId, personId]);
    if (counts.rows[0]?.unready_rows !== 2 || counts.rows[0]?.rules_rows !== 3 || counts.rows[0]?.serves_rows !== 1) {
      throw new Error(`unexpected Stage 2 semantic probe counts: ${JSON.stringify(counts.rows[0])}`);
    }
  } finally {
  ...
  console.log(JSON.stringify({
    marker: 'ATLAS_STAGE2_ACTIVITY_SEMANTIC_KEY_V2',
    status: 'PASS',
    ...
    certainty_excluded_from_identity: true,
    confidence_excluded_from_identity: true,
    notes_excluded_from_identity: true,
    provenance_excluded_from_identity: true,
