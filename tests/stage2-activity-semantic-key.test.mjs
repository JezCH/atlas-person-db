...
const semantics = require('../scripts/stage2-activity-semantic-key-v2.cjs');
...
test('temporal shape and historical year zero are fail-closed', () => {
  assert.throws(
...
test('known same-calendar month/day reversal is rejected while unresolved precision is not fabricated', () => {
  assert.throws(
    () => semantics.semanticKey({
      ...BASE,
      activity_start: 1946,
      activity_start_month: 10,
      activity_start_day: 15,
      activity_start_granularity: 'day',
      activity_end: 1946,
