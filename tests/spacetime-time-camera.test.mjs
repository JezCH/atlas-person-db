...
test("time camera keeps the current surface and removes the legacy height preset control", () => {
  assert.doesNotThrow(() => new Function(viewSource));
  assert.match(viewSource, /createSpacetimeTimeProjection\(/);
...
test("changing time zoom can preserve the historical ordinal under a viewport anchor", () => {
  ...
  const zoomed = model.createSpacetimeTimeProjection(-3000, 2026, 4200 * 1.35, 420);
  ...
  assert.ok(Math.abs(recoveredNew - anchorOrdinal) < 1e-8);
