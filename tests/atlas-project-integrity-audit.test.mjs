...
function baselineFixture() {
  return {
    ...
    datasets: {
      ...
      governance_contexts: [], governance_context_names: [],
      polity_designations: [], polity_designation_names: [],
      people_groups: [], people_group_names: [],
...
test('does not treat distinct Activity identities as duplicates', () => {
  assert.deepEqual(exactActivityDuplicateGroups(baselineFixture().datasets.activities), []);
});

test('detects exact Activity semantic duplicates independent of Activity UUID', () => {
  const source = baselineFixture().datasets.activities[0];
  const groups = exactActivityDuplicateGroups([source, { ...source, id: 'activity-copy' }]);
