...
test("below the internal safety floor time stages are rejected", () => {
...
test("space header is fixed to reviewed subregion hierarchy at the readable floor", () => {
  ...
  assert.equal(plan.minimum_zoom, 0.5);
...
test("spatial hierarchy is leaf-uniform, density-independent, and map-like at the eastern edge", () => {
  ...
  for (const macro of spaceAxis.DEFAULT_SPATIAL_HIERARCHY) {
    ...
    assert.ok(Math.abs((band.max_space - band.min_space) - macro.subregions.length / 45) < 1e-12);
  }
  assert.deepEqual(
  ...
  assert.deepEqual(
    ...
    ["western-central-asia", "western-siberia", "eastern-central-asia-steppe", "tibetan-plateau"]
  );
  assert.deepEqual(
