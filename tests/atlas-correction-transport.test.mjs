...
test("correction handler accepts exact Production SHA, bounded correction sources, explicit mode, and pathless Baseline A capture", () => {
  ...
  assert.deepEqual(correctionHandler.requirePayload({
    ...
    deploymentSha: SHA,
    sourcePath: "corrections/requests/r0.json",
    mode: "dry_run",
    manifest,
  ...
  assert.equal(correctionHandler.requirePayload({
    deployment_sha: SHA,
    intent_path: "corrections/intents/r1.json",
    mode: "apply",
  ...
  assert.throws(() => correctionHandler.requirePayload({ deployment_sha: SHA, mode: "full_activity_baseline", activity_ids: [] }), /CORRECTION_BASELINE_INPUTS_FORBIDDEN/);
...
test("dry-run path does not apply schema migration; apply path does; Baseline A remains read-only", () => {
  ...
  assert.match(handlerSource, /payload\.mode === "full_activity_baseline"/);
...
test("workflow runs only for reviewed correction inputs, enforces per-item dry-run before apply, then captures Baseline A", () => {
  ...
  const baselineCall = workflow.indexOf('mode:"full_activity_baseline"');
  assert.ok(dryRunCall >= 0 && applyCall > dryRunCall);
  assert.ok(baselineCall > applyCall);

  assert.match(workflow, /\.dry_run == true and \.committed == false/);
  assert.match(workflow, /\.dry_run == false and \.committed == true/);
  assert.match(workflow, /ATLAS_CORRECTION_BASELINE_A_V1/);
