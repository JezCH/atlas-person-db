...
function allowedSchemas(sql) {
  return [...sql.matchAll(/'atlas-correction-manifest\/v(?:1(?:\.[123])?|2)'/g)].map((match) => match[0].slice(1, -1));
}
...
test('correction migrations remain replay-monotonic across every registered ledger schema', () => {
  for (const sql of [v11, v2, v12, v13]) assert.deepEqual(allowedSchemas(sql), expected);
