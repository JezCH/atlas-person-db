...
const canonicalReadiness = fs.readFileSync("server/atlas-canonical-data-readiness.js", "utf8");
const reviewedExceptions = JSON.parse(fs.readFileSync("stage2/contracts/p11-reviewed-semantic-v2-exceptions.v1.json", "utf8"));

test("reviewed unresolved relation remains permitted while relation-without-polity stays protected", () => {
...
test("reviewed-null relation constraint migration remains in the current correction migration path", () => {
  const oldFinal = correctionMigrations.indexOf("20260824_person_polity_community_final_corrections.sql");
  const reviewedNullFix = correctionMigrations.indexOf("20260906_p11_reviewed_null_relation_constraint.sql");
  assert.ok(oldFinal >= 0);
  assert.ok(reviewedNullFix > oldFinal);
});
...
test("canonical readiness remains the allowlist gate for unresolved reviewed relations", () => {
  ...
  assert.match(canonicalReadiness, /semantic_v2_blocking_incomplete/);
  assert.match(canonicalReadiness, /ACTIVITY_SEMANTIC_V2_BLOCKING_INCOMPLETE/);
});
