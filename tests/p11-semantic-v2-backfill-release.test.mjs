...
test('P11 semantic-v2 backfill release is exact-SHA, dry-run-first, bounded, and non-destructive', () => {
  ...
  assert.equal(release.execution_request, 'p11-semantic-v2-backfill-file-payload-retry-20260817');
  ...
  assert.match(workflow, /\/tmp\/atlas-p11-backfill\/payloads/);
  assert.match(workflow, /--data-binary\s+"@\$\{payload_file\}"/);
  assert.doesNotMatch(workflow, /--data\s+"\$payload"/);
