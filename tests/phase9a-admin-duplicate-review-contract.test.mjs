...
test('schema self-check validates required column names and types instead of a brittle raw count', () => {
  for (const required of [
    'person_low_id','person_high_id','candidate_state','current_decision','confidence',
    'evidence_fingerprint','decision_evidence_fingerprint','detector_version','review_count',
    'candidate_id','evidence_snapshot','reviewer_kind','request_id'
...
test('Phase 9A review service remains non-destructive even after Phase 9B executor is added separately', () => {
...
test('admin UI keeps review approval separate from explicit survivor-selected merge execution', () => {
  ...
  assert.match(admin, /병합 승인은 판정 기록/);
