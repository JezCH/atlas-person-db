...
test('Runtime compiler remains behind mutation authorization', () => {
  const handlerSource = fs.readFileSync(new URL('../server/atlas-runtime-compile-handler.js', import.meta.url), 'utf8');
  const apiSource = fs.readFileSync(new URL('../api/atlas-runtime-compile.js', import.meta.url), 'utf8');
  assert.match(handlerSource, /createMutationAuthorizer/);
  assert.match(handlerSource, /method\s*!==\s*"POST"/);
  assert.match(apiSource, /createRuntimeCompileHandler/);
});
