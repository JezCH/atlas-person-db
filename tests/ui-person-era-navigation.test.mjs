...
test('era navigation consumes rendered era bands instead of creating a second chronology model', () => {
  ...
  assert.match(eraModel, /const ERAS = Object\.freeze\(\[/);
});
...
test('era navigation assets load after shared era model/table grouping and before Person Main initializes', () => {
  ...
  for (const asset of [eraJs, tableJs, navJs, mainJs, paletteCss, navCssAsset, geometryCss]) assert.ok(html.includes(asset));
