...
test("500 percent uniform time projection is linear reversible and has no year zero", () => {
  const p=timeProjection.createUniformTimeProjection(-3000,2026,4200*5*0.748,5);
...
test("spatial continuum stays leaf-uniform and independent of data density", () => {
  ...
  assert.equal(c.subregions.length,45);
  for(const band of c.subregions) almostEqual(band.max_space-band.min_space,1/45,1e-12);
  for(const macro of spaceAxis.DEFAULT_SPATIAL_HIERARCHY) {
    const band=c.bandForCode(macro.code);
    almostEqual(band.max_space-band.min_space,macro.subregions.length/45,1e-12);
  }
});
