...
test('Vercel exposes exactly the three current ATLAS API entrypoints', () => {
  assert.deepEqual(apiFiles, ['atlas-mutate.js', 'atlas-read.js', 'atlas-session.js']);
});

test('mutation and direct-read entrypoints use server handlers and pg clients only', () => {
  ...
  for (const source of [mutateApi, readApi]) {
...
test('browser pages do not load server entrypoints or pg', () => {
  for (const html of [index, admin]) {
    assert.doesNotMatch(html, /atlas-(?:mutate|read|session)\.js/);
