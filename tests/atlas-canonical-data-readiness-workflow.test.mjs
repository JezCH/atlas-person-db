...
test("canonical readiness replaces the active P11 release-era rehearsal", () => {
  ...
  assert.match(script, /inspectBaselineBReadiness/);
  assert.match(script, /ATLAS_CANONICAL_DATA_READINESS_OK/);
  assert.doesNotMatch(script, /atlas-p11-baseline-b-production-service|captureProductionBaselineB|captureBaselineB|buildBaselineBDocument/);
});
