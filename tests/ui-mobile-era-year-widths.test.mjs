...
test('mobile table uses narrower era and year columns', () => {
  assert.match(css, /@media \(max-width: 760px\)/);
  assert.match(css, /--era-band-width:\s*42px/);
  assert.match(css, /136px 120px minmax\(390px, 1fr\) 66px/);
  assert.match(css, /@media \(max-width: 520px\)/);
  assert.match(css, /--era-band-width:\s*36px/);
  assert.match(css, /128px 112px minmax\(375px, 1fr\) 62px/);
...
test('mobile width override loads after the base table stylesheet', () => {
  ...
  const override = 'atlas-person-mobile-column-widths.css?v=20260816-mobile-widths-v1';
