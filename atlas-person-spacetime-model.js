((root, factory) => {
  ...
  function clampHistoricalOrdinal(ordinal, fallbackYear) {
    return Number.isFinite(ordinal) ? ordinal : historicalYearToOrdinal(fallbackYear);
  }
  ...
  function resolveActivityPlacement(activity, capitalLookup) {
    ...
    if (!interval) {
      return Object.freeze({ activity_id: activityId, polity_id: polityId, status: "chronology_unresolved", segments: Object.freeze([]) });
    }
    if (!polityId) {
    ...
    const activityStart = interval.start_ordinal == null ? interval.end_ordinal : interval.start_ordinal;
    const activityEnd = interval.end_ordinal == null ? interval.start_ordinal : interval.end_ordinal;
    const segments = [];
    for (const period of periods) {
      ...
      segments.push(Object.freeze({
        ...
        start_year: ordinalToHistoricalYear(clampHistoricalOrdinal(overlapStart, interval.start_year || interval.end_year)),
        end_year: ordinalToHistoricalYear(clampHistoricalOrdinal(overlapEnd, interval.end_year || interval.start_year)),
        partial_activity_interval: interval.partial === true
      }));
    }
    if (!segments.length) {
